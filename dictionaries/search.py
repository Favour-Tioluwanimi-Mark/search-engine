...
def article_length(max_length, article_titles, title_to_info):
    ...
    for titles in article_titles:
        if max_length >= title_to_info[titles]['length']:
...
def key_by_author(article_titles, title_to_info):
    result = {}
    for titles in article_titles:
        if title_to_info[titles]['author'] not in result:
            result[title_to_info[titles]['author']] = [titles]
        else:
            result[title_to_info[titles]['author']].append(titles)
    return result
...
def filter_to_author(author, article_titles, title_to_info):
    result = []
    for titles in article_titles:
        if title_to_info[titles]['author'] == author:
            result.append(titles)
    return result
...
def filter_out(keyword, article_titles, keyword_to_titles):
    ...
    for titles in article_titles:
        if titles not in keyword_to_titles[keyword]:
            result.append(titles)
    return result
...
def articles_from_year(year, article_titles, title_to_info):
    year_beginning = datetime.date(year, 1, 1)
    beginning_unix_timestamp = time.mktime(year_beginning.timetuple())

    year_ending = datetime.date(year, 12, 31)
    ending_unix_timestamp = time.mktime(year_ending.timetuple())
    
    result = []
    for titles in article_titles:
        if title_to_info[titles]['timestamp'] >= beginning_unix_timestamp and title_to_info[titles]['timestamp'] <= ending_unix_timestamp:
            result.append(titles)
    return result
