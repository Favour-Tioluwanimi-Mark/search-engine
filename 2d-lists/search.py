...
def unique_authors(count, metadata):
    ...
    new_list = []
    for article in metadata:
        if article[1] not in mentioned_authors:
            new_list.append(article)
            mentioned_authors.append(article[1])
    new_list = new_list[0:count]
    return new_list
...
def title_and_author(metadata):
    new_list = []
    for article in metadata:
        title_and_author = (article[0], article[1])
        new_list.append(title_and_author)
    return new_list
...
def refine_search(keyword, metadata):
    search_two = search(keyword)
    new_list = []
    for article in metadata:
        if article in search_two:
            new_list.append(article)
    return new_list
